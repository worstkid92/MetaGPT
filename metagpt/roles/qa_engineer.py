...
from metagpt.actions import DebugError, RunCode, WriteTest,DeployUK,BuildUK,WriteIntegrationTest,WriteCodeReview,WriteCode
...
class QaEngineer(Role):
    ...
    def __init__(self, **kwargs):
        ...
        self._watch([SummarizeCode, WriteTest, RunCode, DebugError,WriteCodeReview,BuildUK])
        self.test_round = 0
        logger.info(f"Initializeing qa engineer")

    async def _write_test(self, message: Message) -> None:
        src_file_repo = self.project_repo.with_src_path(self.context.src_workspace).srcs
        changed_files = Documents()
        logger.info("in write test")
    ...
    async def _act(self) -> Message:
        ...
        test_filters = any_to_str_set({WriteIntegrationTest, DebugError,WriteCode,WriteCodeReview})
